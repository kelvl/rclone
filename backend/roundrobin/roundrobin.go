...
func init() {
	fsi := &fs.RegInfo{
		Name:        "roundrobin",
		Description: "Roundrobins upstream remotes with a list of binds",
		NewFs:       NewFs,
		Options: []fs.Option{{
			Name:     "remotes",
		...
		{
			Name: "binds",
			Help: "List of space separated addresses.",
			Required: true,
		}},
	}
	fs.Register(fsi)
...
type Options struct {
	...
	Binds fs.SpaceSepList `config:"binds"`
}
...
type Fs struct {
	...
	binds 	 []net.IP     // slice of IP addresses to bind to
}
...
func (f *Fs) String() string {
	return fmt.Sprintf("roundrobin root '%s'", f.root)
}
...
func (f *Fs) Hashes() hash.Set {
	...
	return f.remotes[0].Hashes()
	// return hash.Set(hash.None)
}

// Mkdir makes the root directory of the Fs object
func (f *Fs) Mkdir(dir string) error {
	return f.remotes[0].Mkdir(dir)
...
func (f *Fs) Put(in io.Reader, src fs.ObjectInfo, options ...fs.OpenOption) (fs.Object, error) {
	fs.Debugf(f, "Put into %q", f.remotes[0].String())
	return f.remotes[0].Put(in, src, options...)
...
func (f *Fs) NewObject(path string) (fs.Object, error) {
	return f.remotes[0].NewObject(path)
...
func NewFs(name, root string, m configmap.Mapper) (fs.Fs, error) {
	...
	if len(opt.Remotes) == 0 {
		return nil, errors.New("roundrobin can't point to an empty remote - check the value of the remotes setting")
	}
	if len(opt.Remotes) == 1 {
		return nil, errors.New("roundrobin can't point to a single remote - check the value of the remotes setting")
	}
	for _, remote := range opt.Remotes {
		if strings.HasPrefix(remote, name+":") {
			return nil, errors.New("can't point roundrobin remote at itself - check the value of the remote setting")
		}
	}
	...
	if len(opt.Binds) != len(opt.Remotes) {
		return nil, errors.New("roundrobin requires same number of binds as remotes")
	}
	var binds []net.IP
	for _, bindAddr := range opt.Binds {
		addrs, err := net.LookupIP(bindAddr)
		if err != nil {
			return nil, fmt.Errorf("roundrobin failed to parse %q as IP address: %v", bindAddr, err)
		}
		binds = append(binds, addrs[0])
	}
	...
	f := &Fs{
		...
		remotes: remotes,
		binds: binds,
		features: remotes[0].Features(),
	}

	var sigs []string
	for i, fs := range remotes {
		addr := binds[i]
		sigs = append(sigs, fmt.Sprintf("%s:%s", fs.String(), addr.String()))
	}
	fs.Debugf(f, "Initialized roundrobin with %s", strings.Join(sigs, " "))
